...
export function DisplayProvider({children}: {children :ReactNode}) {
        ...
        calculationArray.forEach( (item: number|string, index) => {
            ...
            if(typeof item == "number" || item == ",") {
                ...
                if(calculationArrayParsed.length > 2){
                    ...
                    const lastArrayItem:number|string = calculationArrayParsed[ calculationArrayParsed.length -1]
    ...
    function handleDisplay (value: Number|string) {
        ...
        const textDisplay = calculationArray.reduce((acc:string, value) => acc + value, '')
    ...
    return(
        <DisplayContext.Provider value={{display, handleDisplay, calculationResult, clearDisplay}}>
            {children}
